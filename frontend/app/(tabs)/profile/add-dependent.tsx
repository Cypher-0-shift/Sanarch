    ...
    <View className="flex-1 px-6 pt-6">
      ...
      <View className="flex-row flex-wrap gap-3">
        {['child', 'parent', 'spouse', 'sibling', 'other'].map(rel => (
          ...
          >
            {({ pressed }) => (
              <View className={`px-5 py-3 rounded-2xl border-2 ${relation === rel ? 'border-[#004D36]' : 'border-[#E5E2DE]'} ${pressed ? 'bg-[#DCF7E3]' : (relation === rel ? 'bg-[#E8F5E9]' : 'bg-white')}`}>
      ...
      {relation === 'other' && (
        <View className="mt-6">
          ...
          <TextInput
            ...
            className="bg-white rounded-2xl px-5 py-4 text-base text-[#2D3A2F] font-display-medium border border-[#E5E2DE]"
          />
        </View>
      )}
      ...
      <View className="flex-1 bg-black/50 justify-end">
        <View className="bg-white rounded-t-[32px] p-6 pb-10 max-h-[80%]">
          <View className="flex-row justify-between items-center mb-6">
            ...
            <TouchableOpacity onPress={() => setVisible(false)} className="p-2 bg-[#F5F3F0] rounded-full">
